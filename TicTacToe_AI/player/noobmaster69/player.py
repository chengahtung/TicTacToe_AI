...
class Player:
    ...
    def isWinner(self,board, let):
        #priority PLAYER win
        return ((board[0] == let and board[1] == let and board[2] == let and board[3] == let) or
            (board[4] == let and board[5] == let and board[6] == let and board[7] == let) or
            (board[8] == let and board[9] == let and board[10] == let and board[11] == let) or
            (board[12] == let and board[13] == let and board[14] == let and board[15] == let) or
            (board[0] == let and board[4] == let and board[8] == let and board[12] == let) or
            (board[1] == let and board[5] == let and board[9] == let and board[13] == let) or
            (board[2] == let and board[6] == let and board[10] == let and board[14] == let) or
            (board[3] == let and board[7] == let and board[11] == let and board[15] == let) or
            (board[0] == let and board[5] == let and board[10] == let and board[15] == let) or
            (board[3] == let and board[6] == let and board[9] == let and board[12] == let))
    ...
    def play(self, game_board_state, player_n):
        ...
        for i in [0, 5, 10, 15, 3, 6, 9, 12]:
              ...
              if self.getDiagonaldownright(game_board_state, i):
                 move = i
                 break
              #check for diagonal empty spaces in current board
              if self.getDiagonaldownleft(game_board_state, i):
                 move = i
                 break

              for o in possibleMoves:
                  #using the virtual board with the moves inserted, check to see if hasScore
                  if self.hasScore(boardCopy, o,player_n):
                      move = o
                      break
